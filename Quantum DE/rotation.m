...
%[angle] = look1(A1,D,n,popSize);
%[angle] = look2(A1,D,n,popSize);
%[angle] = look3(A1,compArray1,bst1,compbst1,n,popSize,D); %combinatn D&bst
for k=1:popSize
    for  i=1:n
        for j=1:n
            G=[cos(angle(i,j,k)),-sin(angle(i,j,k));sin(angle(i,j,k)),cos(angle(i,j,k))];
            L=[cos(-angle(i,j,k)),-sin(-angle(i,j,k));sin(-angle(i,j,k)),cos(-angle(i,j,k))];
            H=[alpha(i,j,k);beta(i,j,k)];
            if ((alpha(i,j,k)>0)&&beta(i,j,k)>0)|| ((alpha(i,j,k)<0)&&beta(i,j,k)<0)% first and third quadrant
                positions=G*H;
            else
                positions= L*H;% if in second and fourth quadrant
            end
